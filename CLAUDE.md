...
## Laravel Boost MCP tools are currently broken

**Boost's MCP tools do not work.** `database-query`, `database-schema`, `database-connections`, `search-docs`, `tinker`, `browser-logs`, `last-error`, `read-log-entries`, `application-info`, and `get-absolute-url` all fail. Boost's artisan commands (`boost:install`, `boost:update`, skills, guidelines) are unaffected.

**This overrides the Boost guidelines below**, which are generated by `boost:update` and still say to prefer Boost tools and to always run `search-docs` first. Ignore those two instructions until this is resolved. Use the ordinary alternatives: `php artisan tinker --execute '…'` for queries and app-context PHP, `php artisan route:list` / `config:show` for app metadata, and `storage/logs` for logs.

**Why:** Composer installs one `laravel/mcp` for the whole project, shared between the app (`require`) and Boost (`require-dev`). This app pins `laravel/mcp` to `dev-main` to get an unreleased OAuth fix (see [wayfinder map #180](https://github.com/davidharting/davidharting.com/issues/180)), and `laravel/mcp` 1.x serves only MCP protocol `2026-07-28`, having dropped the `initialize` handshake. Boost's MCP server inherits that and rejects the legacy handshake its clients send. No Boost release supports 1.x, so it cannot be fixed from this side.

**When it is fixed:** delete this section. Either Boost ships 1.x support, or #194 moves `laravel/mcp` back to a tagged release and this is re-evaluated then.
